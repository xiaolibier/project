...
!function (window, undefined) {
  ...
  function _each(a, b) {
    if (_isArray(a)) for (var c = 0, d = a.length; d > c && b.call(a[c], c, a[c]) !== !1; c++);
  ...
  function _bind(a, b, c) {
    ...
    e && e.length > 0 ? _unbindEvent(a, b, e[0])  : (_eventData[d][b] = [
    ], _eventData[d].el = a),
    e = _eventData[d][b],
  ...
  function _formatUrl(a, b, c, d) {
    ...
    if (b = _undef(b, '').toLowerCase(), 'data:' != a.substr(0, 5) && (a = a.replace(/([^:])\/\//g, '$1/')), _inArray(b, [
      'absolute',
      'relative',
      'domain'
    ]) < 0) return a;
  ...
  function _formatHtml(a, b, c, d, e) {
    ...
    return a = a.replace(h, function (a, h, j, k, l, m, n) {
      ...
      if ('' === t && _SINGLE_TAG_MAP[r] && (t = ' /'), _INLINE_TAG_MAP[r] && (p && (p = ' '), u && (u = ' ')), _PRE_TAG_MAP[r] && (q ? u = '\n' : p = '\n'), d && 'br' == r && (u = '\n'), _BLOCK_TAG_MAP[r] && !_PRE_TAG_MAP[r]) if (d) {
        ...
        p = '\n',
        u = '\n';
        for (var v = 0, w = q ? i.length : i.length - 1; w > v; v++) p += e,
        q || (u += e);
        t ? i.pop()  : q || (u += e)
      } else p = u = '';
      if ('' !== s) {
        ...
        _each(x, function (a, d) {
          if (_FILL_ATTR_MAP[a] && (x[a] = a), _inArray(a, [
            'src',
            'href'
          ]) >= 0 && (x[a] = _formatUrl(d, c)), (b && 'style' !== a && !g[r]['*'] && !g[r][a] || 'body' === r && 'contenteditable' === a || /^kindeditor_\d+$/.test(a)) && delete x[a], 'style' === a && '' !== d) {
            var e = _getCssList(d);
            _each(e, function (a) {
  ...
  function _queryAll(a, b) {
    ...
    function h(a, b, c) {
      for (var f, g = [
      ], h = c.ownerDocument || c, i = h.getElementsByName(d(a)), j = 0, k = i.length; k > j; j++) f = i[j],
      e(b, f.nodeName) && _contains(c, f) && null !== f.getAttribute('name') && g.push(f);
      return g
    }
    function i(a, b, d, e) {
      for (var f, g = [
      ], h = e.getElementsByTagName(d), i = 0, j = h.length; j > i; i++) f = h[i],
    ...
    b = b || document;
    for (var m, n = [
    ], o = /((?:\\.|[^\s>])+|[\s>])/g; m = o.exec(a); ) ' ' !== m[1] && n.push(m[1]);
    var p = [
    ];
    ...
    for (v = 0, lenth = n.length; v < lenth; v++) if (q = n[v], '>' !== q) {
      if (v > 0) {
        for (r = [
        ], w = 0, y = p.length; y > w; w++) for (t = p[w], s = j(q, t), x = 0, z = s.length; z > x; x++) u = s[x],
  ...
  function _singleKeyMap(a) {
    ...
    return _each(a, function (a, e) {
      b = a.split(',');
      for (var f = 0, g = b.length; g > f; f++) c = b[f],
      d[c] = e
    }),
    d
  }
  ...
  function _create(a, b) {
    function c(a) {
      return _each(_plugins, function (b, c) {
        _isFunction(c) && (c.call(a, KindEditor), a._pluginStatus || (a._pluginStatus = {
        }), a._pluginStatus[b] = 'inited')
      }),
      a.create()
    }
    if (b = b || {
    ...
    if (e && 0 !== e.length) {
      ...
      _language[f.langType] ? c(f)  : (_loadScript(f.langPath + f.langType + '.js?ver=' + encodeURIComponent(K.DEBUG ? _TIME : _VERSION), function () {
        c(f)
      }), f)
    }
  }
  ...
  if (!window.KindEditor) {
    ...
    _extend(KEvent, {
    ...
    }, _readyFinished = !1; window.attachEvent && window.attachEvent('onunload', function () {
      _each(_eventData, function (a, b) {
        b.el && _unbind(b.el)
      })
    }), K.ctrl = _ctrl, K.ready = _ready, K.formatUrl = _formatUrl, K.formatHtml = _formatHtml, K.getCssList = _getCssList, K.getAttrList = _getAttrList, K.mediaType = _mediaType, K.mediaAttrs = _mediaAttrs, K.mediaEmbed = _mediaEmbed, K.mediaImg = _mediaImg, K.clearMsWord = _clearMsWord, K.tmpl = _tmpl; var _getSetAttrDiv = document.createElement('div'); _getSetAttrDiv.setAttribute('className', 't'); var _GET_SET_ATTRIBUTE = 't' !== _getSetAttrDiv.className; K.query = _query, K.queryAll = _queryAll, _extend(KNode, {
      init: function (a) {
        var b = this;
        a = _isArray(a) ? a : [
          a
      ...
      clone: function (a) {
        return new KNode(this.length < 1 ? [
        ] : this[0].cloneNode(a))
      },
      append: function (a) {
        return this.each(function () {
      ...
      children: function () {
        ...
        for (var a = [
        ], b = this[0].firstChild; b; ) (3 != b.nodeType || '' !== _trim(b.nodeValue)) && a.push(b),
        b = b.nextSibling;
        return new KNode(a)
      },
      first: function () {
      ...
      K[a] = b
    }), K.NodeClass = KNode, window.KindEditor = K; var _START_TO_START = 0, _START_TO_END = 1, _END_TO_END = 2, _END_TO_START = 3, _BOOKMARK_ID = 0; _extend(KRange, {
      init: function (a) {
        var b = this;
        b.startContainer = a,
        b.startOffset = 0,
    ...
    }), K.WidgetClass = KWidget, K.widget = _widget; var html, _direction = ''; (html = document.getElementsByTagName('html')) && (_direction = html[0].dir), _extend(KEdit, KWidget, {
      init: function (a) {
        function b() {
      ...
      disable: function (a) {
        var b = this,
        c = a.get ? a : b.get(a);
        return c && (c.removeClass('ke-selected').addClass('ke-disabled'), c.opacity(0.5)),
        b
      },
      ...
      init: function (a) {
        ...
        var l = [
          '<div class="ke-inline-block ' + h + '">',
          a.target ? '' : '<iframe name="' + i + '" style="display:none;"></iframe>',
          a.form ? '<div class="ke-upload-area">' : '<form class="ke-upload-area ke-form" method="post" enctype="multipart/form-data" target="' + i + '" action="' + e + '">',
          '<span class="ke-button-common">',
          j.join(''),
          '<input type="button" class="ke-button-common ke-button" value="' + f + '" />',
          '</span>',
          '<input type="file" class="ke-upload-file" name="' + d + '" tabindex="-1" />',
          a.form ? '</div>' : '</form>',
          '</div>'
        ].join(''),
        m = K(l, c.doc);
    ...
    }), K.DialogClass = KDialog, K.dialog = _dialog, K.tabs = _tabs, K.loadScript = _loadScript, K.loadStyle = _loadStyle, K.ajax = _ajax; var _plugins = {
    }, _language = {
    }; KEditor.prototype = {
      lang: function (a) {
      ...
      handler: function (a, b) {
        ...
        _isFunction(b) ? (c._handlers[a].push(b), c)  : (_each(c._handlers[a], function () {
          b = this.call(c, b)
        }), b)
      },
      ...
      remove: function () {
        ...
        return a.isCreated ? (a.beforeRemove(), a.srcElement.data('kindeditor', ''), a.menu && a.hideMenu(), _each(a.dialogs, function () {
          a.hideDialog()
        }), K(document).unbind('mousedown', a._docMousedownFn), a.toolbar.remove(), a.edit.remove(), a.statusbar.last().unbind(), a.statusbar.unbind(), a.container.remove(), a.container = a.toolbar = a.edit = a.menu = null, a.dialogs = [
        ], a.isCreated = !1, a)  : a
      },
      ...
      readonly: function (a) {
        ...
        c = b.edit,
        d = c.doc;
        b.designMode ? b.toolbar.disableAll(a, [
        ])  : _each(b.noDisableItems, function () {
      ...
      _each('prop,cellprop,colinsertleft,colinsertright,rowinsertabove,rowinsertbelow,rowmerge,colmerge,rowsplit,colsplit,coldelete,rowdelete,insert,delete'.split(','), function (a, c) {
        var d = _inArray(c, [
          'prop',
          'delete'
        ]) < 0 ? b.plugin.getSelectedCell : b.plugin.getSelectedTable;
        b.addContextmenu({
...
KindEditor.plugin('anchor', function (a) {
  var b = this,
  c = 'anchor',
  d = b.lang(c + '.');
  b.plugin.anchor = {
...
KindEditor.plugin('baidumap', function (a) {
  ...
  b.clickToolbar(c, function () {
    ...
    j = [
      ...
      '<div class="ke-header">',
      '<div class="ke-left">',
      d.address + ' <input id="kindeditor_plugin_map_address" name="address" class="ke-input-text" value="" style="width:200px;" /> ',
      '<span class="ke-button-common ke-button-outer">',
      '<input type="button" name="searchBtn" class="ke-button-common ke-button" value="' + d.search + '" />',
      '</span>',
      '</div>',
      '<div class="ke-right">',
      '<input type="checkbox" id="keInsertDynamicMap" name="insertDynamicMap" value="1" /> <label for="keInsertDynamicMap">' + d.insertDynamicMap + '</label>',
      '</div>',
      '<div class="ke-clearfix"></div>',
      '</div>',
      '<div class="ke-map" style="width:' + e + 'px;height:' + f + 'px;"></div>',
      '</div>'
    ].join(''),
    k = b.createDialog({
      name: c,
      ...
      yesBtn: {
        ...
        click: function () {
          ...
          g = a.getZoom(),
          i = [
            o[0].checked ? b.pluginsPath + 'baidumap/index.html' : 'http://api.map.baidu.com/staticimage',
            '?center=' + encodeURIComponent(d),
            '&zoom=' + encodeURIComponent(g),
            '&width=' + e,
            '&height=' + f,
            '&markers=' + encodeURIComponent(d),
...
KindEditor.plugin('map', function (a) {
  ...
  b.clickToolbar(c, function () {
    ...
    '</style>',
    '<script src="http://maps.googleapis.com/maps/api/js?sensor=false&language=' + b.langType + '"></script>',
    '<script>',
    'var map, geocoder;',
    'function initialize() {',
...
KindEditor.plugin('filemanager', function (a) {
  ...
  d.plugin.filemanagerDialog = function (b) {
    function i(b, c, e) {
      ...
      a.ajax(a.addParam(f, g + '&' + (new Date).getTime()), function (a) {
        t.hideLoading(),
        e(a)
      })
    }
    ...
    function l(b) {
      ...
      for (var d = b.file_list, e = 0, f = d.length; f > e; e++) {
        ...
        !i.is_dir || i.has_file ? (m.css('cursor', 'pointer'), p.attr('title', i.filename), j(p, b, i, l))  : p.attr('title', h.emptyFolder),
    ...
    function m(b) {
      ...
      for (var d = b.file_list, e = 0, f = d.length; f > e; e++) {
        ...
        !i.is_dir || i.has_file ? (n.css('cursor', 'pointer'), c(n, i), j(n, b, i, m))  : n.attr('title', h.emptyFolder),
        n.append(q),
    ...
    s = [
      ...
      '<div class="ke-left">',
      '<img class="ke-inline-block" name="moveupImg" src="' + g + 'go-up.gif" width="16" height="16" border="0" alt="" /> ',
      '<a class="ke-inline-block" name="moveupLink" href="javascript:;">' + h.moveup + '</a>',
      '</div>',
      '<div class="ke-right">',
      h.viewType + ' <select class="ke-inline-block" name="viewType">',
      '<option value="VIEW">' + h.viewImage + '</option>',
      '<option value="LIST">' + h.listImage + '</option>',
      '</select> ',
      h.orderType + ' <select class="ke-inline-block" name="orderType">',
  ...
  b.plugin.flash = {
    edit: function () {
      var k = [
        '<div style="padding:20px;">',
        '<div class="ke-dialog-row">',
        '<label for="keUrl" style="width:60px;">' + d.url + '</label>',
        '<input class="ke-input-text" type="text" id="keUrl" name="url" value="" style="width:160px;" /> &nbsp;',
        '<input type="button" class="ke-upload-button" value="' + d.upload + '" /> &nbsp;',
        '<span class="ke-button-common ke-button-outer">',
        '<input type="button" class="ke-button-common ke-button" name="viewServer" value="' + d.viewServer + '" />',
        '</span>',
        '</div>',
        '<div class="ke-dialog-row">',
        '<label for="keWidth" style="width:60px;">' + d.width + '</label>',
        '<input type="text" id="keWidth" class="ke-input-text ke-input-number" name="width" value="550" maxlength="4" /> ',
        '</div>',
        '<div class="ke-dialog-row">',
        '<label for="keHeight" style="width:60px;">' + d.height + '</label>',
...
KindEditor.plugin('image', function (a) {
  ...
  b.plugin.imageDialog = function (d) {
    ...
    v = [
      ...
      '<div class="ke-dialog-row">',
      '<label for="remoteUrl" style="width:60px;">' + n.remoteUrl + '</label>',
      '<input type="text" id="remoteUrl" class="ke-input-text" name="url" value="" style="width:200px;" /> &nbsp;',
      '<span class="ke-button-common ke-button-outer">',
      '<input type="button" class="ke-button-common ke-button" name="viewServer" value="' + n.viewServer + '" />',
      '</span>',
      '</div>',
      '<div class="ke-dialog-row">',
      '<label for="remoteWidth" style="width:60px;">' + n.size + '</label>',
      n.width + ' <input type="text" id="remoteWidth" class="ke-input-text ke-input-number" name="width" value="" maxlength="4" /> ',
      n.height + ' <input type="text" class="ke-input-text ke-input-number" name="height" value="" maxlength="4" /> ',
      '<img class="ke-refresh-btn" src="' + j + 'refresh.png" width="16" height="16" alt="" style="cursor:pointer;" title="' + n.resetSize + '" />',
      '</div>',
      '<div class="ke-dialog-row">',
      '<label style="width:60px;">' + n.align + '</label>',
      '<input type="radio" name="align" class="ke-inline-block" value="" checked="checked" /> <img name="defaultImg" src="' + j + 'align_top.gif" width="23" height="25" alt="" />',
      ' <input type="radio" name="align" class="ke-inline-block" value="left" /> <img name="leftImg" src="' + j + 'align_left.gif" width="23" height="25" alt="" />',
      ' <input type="radio" name="align" class="ke-inline-block" value="right" /> <img name="rightImg" src="' + j + 'align_right.gif" width="23" height="25" alt="" />',
      '</div>',
      '<div class="ke-dialog-row">',
      '<label for="remoteTitle" style="width:60px;">' + n.imgTitle + '</label>',
      '<input type="text" id="remoteTitle" class="ke-input-text" name="title" value="" style="width:200px;" />',
      '</div>',
      '</div>',
      '<div class="tab2" style="display:none;">',
      '<iframe name="' + r + '" style="display:none;"></iframe>',
      '<form class="ke-upload-area ke-form" method="post" enctype="multipart/form-data" target="' + r + '" action="' + a.addParam(h, 'dir=image') + '">',
      '<div class="ke-dialog-row">',
      s.join(''),
      '<label style="width:60px;">' + n.localUrl + '</label>',
      '<input type="text" name="localUrl" class="ke-input-text" tabindex="-1" style="width:200px;" readonly="true" /> &nbsp;',
      '<input type="button" class="ke-upload-button" value="' + n.upload + '" />',
      '</div>',
      '</form>',
      '</div>',
      '</div>'
...
KindEditor.plugin('insertfile', function (a) {
  ...
  b.plugin.fileDialog = function (k) {
    ...
    o = [
      '<div style="padding:20px;">',
      '<div class="ke-dialog-row">',
      '<label for="keUrl" style="width:60px;">' + j.url + '</label>',
      '<input type="text" id="keUrl" name="url" class="ke-input-text" style="width:160px;" /> &nbsp;',
      '<input type="button" class="ke-upload-button" value="' + j.upload + '" /> &nbsp;',
      '<span class="ke-button-common ke-button-outer">',
      '<input type="button" class="ke-button-common ke-button" name="viewServer" value="' + j.viewServer + '" />',
      '</span>',
      '</div>',
      '<div class="ke-dialog-row">',
      '<label for="keTitle" style="width:60px;">' + j.title + '</label>',
  ...
  b.plugin.media = {
    edit: function () {
      var k = [
        ...
        '</div>',
        '<div class="ke-dialog-row">',
        '<label for="keAutostart">' + d.autostart + '</label>',
...
function (a) {
  ...
  a.extend(b, {
    init: function (b) {
      ...
      d.div = a(b.container).html(['<div class="ke-swfupload">',
      '<div class="ke-swfupload-top">',
      '<div class="ke-inline-block ke-swfupload-button">',
      '<input type="button" value="Browse" />',
      '</div>',
      '<div class="ke-inline-block ke-swfupload-desc">' + b.uploadDesc + '</div>',
      '<span class="ke-button-common ke-button-outer ke-swfupload-startupload">',
      '<input type="button" class="ke-button-common ke-button" value="' + b.startButtonValue + '" />',
      '</span>',
      '</div>',
    ...
    appendFile: function (b) {
      ...
      a(['<div class="ke-progressbar">',
      '<div class="ke-progressbar-bar"><div class="ke-progressbar-bar-inner"></div></div>',
      '<div class="ke-progressbar-percent">0%</div></div>'].join('')).hide().appendTo(g),
...
KindEditor.plugin('multiimage', function (a) {
  var b = this,
  c = 'multiimage',
...
function () {
  ...
  SWFUpload.prototype.getFlashHTML = function () {
    ...
    [
      '<object id="',
      this.movieName,
      '"' + a + ' type="application/x-shockwave-flash" data="',
  ...
  SWFUpload.prototype.setStats = function (a) {
    this.callFlash('SetStats', [
      a
    ])
  },
  SWFUpload.prototype.getFile = function (a) {
    return 'number' == typeof a ? this.callFlash('GetFileByIndex', [
      a
    ])  : this.callFlash('GetFile', [
      a
    ])
  },
  ...
  SWFUpload.prototype.setFileSizeLimit = function (a) {
    ...
    this.callFlash('SetFileSizeLimit', [
      a
    ])
  },
  ...
  SWFUpload.prototype.returnUploadStart = function (a) {
    ...
    void 0 === b && (b = !0),
    b = !!b,
    this.callFlash('ReturnUploadStart', [
      b
    ])
  },
...
function () {
  'function' == typeof SWFUpload && (SWFUpload.queue = {
    ...
    var b,
    c = this.queueSettings.user_upload_complete_handler;
    if (a.filestatus === SWFUpload.FILE_STATUS.COMPLETE && this.queueSettings.queue_upload_count++, b = 'function' == typeof c ? c.call(this, a) === !1 ? !1 : !0 : a.filestatus === SWFUpload.FILE_STATUS.QUEUED ? !1 : !0) {
      var d = this.getStats();
      d.files_queued > 0 && this.queueSettings.queue_cancelled_flag === !1 ? this.startUpload()  : this.queueSettings.queue_cancelled_flag === !1 ? (this.queueEvent('queue_complete_handler', [
        this.queueSettings.queue_upload_count
      ]), this.queueSettings.queue_upload_count = 0)  : (this.queueSettings.queue_cancelled_flag = !1, this.queueSettings.queue_upload_count = 0)
    }
  })
}(),
...
KindEditor.plugin('quickformat', function (a) {
  ...
  c.clickToolbar(d, function () {
    c.focus();
    for (var d, f = c.edit.doc, g = c.cmd.range, h = a(f.body).first(), i = [
    ], j = [
    ], k = g.createBookmark(!0); h; ) {
      d = h.next();
      var l = b(h);
  ...
  e.plugin.table = {
    prop: function (d) {
      var i = [
        '<div style="padding:20px;">',
        '<div class="ke-dialog-row">',
        '<label for="keRows" style="width:90px;">' + g.cells + '</label>',
        g.rows + ' <input type="text" id="keRows" class="ke-input-text ke-input-number" name="rows" value="" maxlength="4" /> &nbsp; ',
        g.cols + ' <input type="text" class="ke-input-text ke-input-number" name="cols" value="" maxlength="4" />',
        '</div>',
        '<div class="ke-dialog-row">',
        '<label for="keWidth" style="width:90px;">' + g.size + '</label>',
        g.width + ' <input type="text" id="keWidth" class="ke-input-text ke-input-number" name="width" value="" maxlength="4" /> &nbsp; ',
        '<select name="widthType">',
        '<option value="%">' + g.percent + '</option>',
        '<option value="px">' + g.px + '</option>',
        '</select> &nbsp; ',
        g.height + ' <input type="text" class="ke-input-text ke-input-number" name="height" value="" maxlength="4" /> &nbsp; ',
        ...
        '</div>',
        '<div class="ke-dialog-row">',
        '<label for="kePadding" style="width:90px;">' + g.space + '</label>',
        g.padding + ' <input type="text" id="kePadding" class="ke-input-text ke-input-number" name="padding" value="" maxlength="4" /> &nbsp; ',
        g.spacing + ' <input type="text" class="ke-input-text ke-input-number" name="spacing" value="" maxlength="4" />',
        '</div>',
        '<div class="ke-dialog-row">',
        '<label for="keAlign" style="width:90px;">' + g.align + '</label>',
        '<select id="keAlign" name="align">',
        '<option value="">' + g.alignDefault + '</option>',
        '<option value="left">' + g.alignLeft + '</option>',
        '<option value="center">' + g.alignCenter + '</option>',
        '<option value="right">' + g.alignRight + '</option>',
        '</select>',
        '</div>',
        '<div class="ke-dialog-row">',
        '<label for="keBorder" style="width:90px;">' + g.border + '</label>',
        g.borderWidth + ' <input type="text" id="keBorder" class="ke-input-text ke-input-number" name="border" value="" maxlength="4" /> &nbsp; ',
        g.borderColor + ' <span class="ke-inline-block ke-input-color"></span>',
        '</div>',
        '<div class="ke-dialog-row">',
        '<label for="keBgColor" style="width:90px;">' + g.backgroundColor + '</label>',
    ...
    cellprop: function () {
      var d = [
        ...
        '<div class="ke-dialog-row">',
        '<label for="keAlign" style="width:90px;">' + g.align + '</label>',
        g.textAlign + ' <select id="keAlign" name="textAlign">',
        '<option value="">' + g.alignDefault + '</option>',
        '<option value="left">' + g.alignLeft + '</option>',
        '<option value="center">' + g.alignCenter + '</option>',
        '<option value="right">' + g.alignRight + '</option>',
        '</select> ',
        g.verticalAlign + ' <select name="verticalAlign">',
        '<option value="">' + g.alignDefault + '</option>',
        '<option value="top">' + g.alignTop + '</option>',
        '<option value="middle">' + g.alignMiddle + '</option>',
        '<option value="bottom">' + g.alignBottom + '</option>',
        '<option value="baseline">' + g.alignBaseline + '</option>',
        '</select>',
        '</div>',
        '<div class="ke-dialog-row">',
        '<label for="keBorder" style="width:90px;">' + g.border + '</label>',
      ...
      k[0].select();
      var u,
      v = e.plugin.getSelectedCell(),
      w = v[0].style.width || v[0].width || '',
      x = v[0].style.height || v[0].height || '';
      (u = /^(\d+)((?:px|%)*)$/.exec(w)) ? (k.val(u[1]), m.val(u[2]))  : k.val(''),
...
KindEditor.plugin('template', function (a) {
  ...
  c.clickToolbar(d, function () {
    ...
    html = [
      ...
      '</select></div>',
      '<div class="ke-right">',
      '<input type="checkbox" id="keReplaceFlag" name="replaceFlag" value="1" /> <label for="keReplaceFlag">' + e.replaceContent + '</label>',
      '</div>',
      '<div class="ke-clearfix"></div>',
      '</div>',
