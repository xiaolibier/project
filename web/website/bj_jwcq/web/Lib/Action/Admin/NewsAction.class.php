		 ...
		 $main_menu=M("main_menu");
		  switch($sbutton)
		  {
		    ///添加信息内容
		      ...
		      echo "<script>alert('信息添加成功!');window.location.href='/Admin/News/caselist?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."';</script>";
			  break;
			case "修改":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if($Mlist!==false) 
					{ 
						 echo "<script>alert('数据修改成功!');window.location.href='/Admin/News/caselist?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."';</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
		 ...
		 $col_news=M("col_news");
		  switch($sbutton)
		  {
		    ///添加信息内容
		      ...
		      echo "<script>alert('添加成功');window.location.href='/Admin/News/caseimgNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."'</script>";
			  break;
			case "修改":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if($Mlist!==false) 
					{ 
						  ...
						  echo "<script>alert('数据修改成功!');window.location.href='/Admin/News/caseimgNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."'</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
		 ...
		 $col_news=M("col_news");
		  switch($sbutton)
		  {
		    ///添加信息内容
			  ...
			  //写入日志
			   add_login($ColTitle, $ustate,$content1,$type="添加");
		      echo "<script>alert('数据添加成功!');window.location.href='/Admin/News/caseNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."'</script>";
			  break;
			case "修改":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if($Mlist!==false) 
					{ 
					      //写入日志
						  add_login($ColTitle, $ustate,$content1,$type="修改");
						  echo "<script>alert('数据修改成功!');window.location.href='/Admin/News/caseNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."';</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
