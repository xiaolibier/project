		 ...
		 $main_menu=M("main_menu");
		  switch($sbutton)
		  {
		    ///添加信息内容
		      ...
		      echo "<script>alert('信息添加成功!');window.location.href='/Admin/Base/caselist?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."';</script>";
			  break;
			case "修改":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if($Mlist!==false) 
					{ 
						 echo "<script>alert('数据修改成功!');window.location.href='/Admin/Base/caselist?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."';</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
...
{
		 $uploads = $this->_upload();
		 $uploadpic = $uploads['pics']; //上传图片
		// $uploadsoft = $uploads['soft']; //上传文件
//		 $uploadvideo = $uploads['video']; //上传视频
		 $sbutton=$this->_request("Submit");
		 $ffaID=$this->_request("ffaids");
		 $uploadsoft=$this->_request("soft1");
		 $uploadvideo=$this->_request("video1");
		 ...
		 $Col_Source=$this->_request("Col_Source");
		 $Col_gnjj=$this->_request("Col_gnjj");
		 $ColEditor=$this->_request("ColEditor");
		 $ColHot=$this->_request("ColHot");
		 $Col_Top=$this->_request("Col_Top");
		 $Col_New=$this->_request("Col_New");
		 ...
		 $content1=$this->_request("content1");
		 $Col_qy=$this->_request("Col_qy");
		 $Col_mj=$this->_request("Col_mj");
		 $col_news=M("col_news");
		  switch($sbutton)
		  {
		    ///添加信息内容
			  ...
			  $data["Col_Source"]= $Col_Source;
		      $data["Col_Editor"]= $ColEditor;
			  $data["Col_New"]= $Col_New;
		      $data["Col_Hot"]= $ColHot;
			  $data["Col_Top"]= $Col_Top;
			  ...
			  $data["Col_Pic"]=$uploadpic;//图片
			  $data["Col_File"]=$uploadsoft;//上传文件
			  $data["Col_Video"]=$uploadvideo;//视频文件
		      ...
		      echo "<script>alert('添加成功');window.location.href='/Admin/Base/caseimgNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."'</script>";
			  break;
			case "修改":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if (!empty($uploads['images']))
					{
						$uploadpic = $uploads['images'];//上传图片
						$Data["Col_Pic"]=$uploadpic; 
					}
				  $Data["Col_File"]=$uploadsoft; 
				  $Data["Col_Video"]=$uploadvideo; 
				  $Data["Col_Click"]= $ColClick;
				  $Data["Col_Hot"]= $ColHot;
				  $Data["Col_Top"]= $Col_Top;
				  $Data["Col_New"]= $Col_New;
				  ...
				  if($Mlist!==false) 
					{ 
						  ...
						  echo "<script>alert('数据修改成功!');window.location.href='/Admin/Base/caseimgNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."'</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
...
function caseimgColCreate()
{
	   ...
	   $main_father=getmain_id($faID);
	    $colnews1=$Theobj->where('Col_MenuID in(229,271) and col_State=1')->order('Col_Order asc,Col_ID desc')->select();
	   if($main_father<> $ffaID)
	   {
	   		$main_names=getmain_names($main_father)." > ";
	   }
	   ...
	   switch($act)
	   {
		  ...
		  case "Edit":
			 ...
			 { 
				...
				$Col_Top=$menu_data[0]["Col_Top"];
			 	$Col_File=$menu_data[0]["Col_File"];
				$Col_New=$menu_data[0]['Col_New'];
				$Col_Video=$menu_data[0]["Col_Video"];
				$this->assign('Col_Pic',$Col_Pic);
				$this->assign('Col_Video',$Col_Video);
				$this->assign('Col_File',$Col_File);
				$this->assign('Col_State',$Col_State);
				$this->assign('col_time',$col_time);
				$this->assign('Col_Hot',$Col_Hot);
				$this->assign('Col_Top',$Col_Top);
				$this->assign('Col_New',$Col_New);
				$this->assign('Col_Order',$Col_Order);
				$this->assign('ColClick',$ColClick);
				$this->assign('bvalue',$bvalue);
		 ...
		 $col_news=M("col_news");
		  switch($sbutton)
		  {
		    ///添加信息内容
			  ...
			  //写入日志
			   add_login($ColTitle, $ustate,$content1,$type="添加");
		      echo "<script>alert('数据添加成功!');window.location.href='/Admin/Base/caseNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."'</script>";
			  break;
			case "修改":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if($Mlist!==false) 
					{ 
					      //写入日志
						  add_login($ColTitle, $ustate,$content1,$type="修改");
						  echo "<script>alert('数据修改成功!');window.location.href='/Admin/Base/caseNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."';</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
      ...
      //  $upload->thumbPrefix = 'm_,s_';  //生产2张缩略图 
       ...
       // $upload->thumbMaxHeight = '400,1000'; //缩略图的最大高度，多个使用逗号分隔
		...
		{ 
			...
			// var_dump($uploadList);
			 //exit();
			// if($uploadList[0]['extension']=="flv")
//			 {
//			 	 var_dump("111");
//			 exit();
//			 }
			 $images1=$this->_request("images1");
			
			 $soft1=$this->_request("soft1");
			 $video1=$this->_request("video1");
			 if($images1)
			 {
            	 $uploads['images'] = $uploadList[0]['savename']; 
			 }
			 if($soft1)
			 {
				 if(!$images1&&!$video1)
				 {
			 	 	$uploads['soft'] = $uploadList[0]['savename'];
				 }
				 else
				 {
				 	$uploads['soft'] = $uploadList[1]['savename'];
				 }
			 }
			 if($video1)
			 {
				 if(!$images1&&!$soft1)
			 		$uploads['video'] = $uploadList[0]['savename'];
				 elseif($images1&&!$soft1)
				 	$uploads['video'] = $uploadList[1]['savename'];
				 else
				 	$uploads['video'] = $uploadList[2]['savename'];
			 }
        } 
