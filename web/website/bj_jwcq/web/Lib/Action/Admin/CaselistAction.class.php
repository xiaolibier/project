...
class CaselistAction extends Action 
{
		 ...
		 $main_menu=M("main_menu");
		  switch($sbutton)
		  {
		    ///添加信息内容
			  ...
			  $data["Main_bm"]= $Main_bm;
		      $data["Main_Father"]=$faID;
			  $data["Main_Language"]=$language;
		      $res=$main_menu->add($data);
			  ///写入操作日志
			  add_login($ColName,$ustate,$content1,$type="添加");
		      echo "<script>alert('信息添加成功!');window.location.href='/Admin/Caselist/caselist?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."';</script>";
			  break;
			case "修改":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if($Mlist!==false) 
					{ 
						 echo "<script>alert('数据修改成功!');window.location.href='/Admin/Caselist/caselist?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."';</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
		 ...
		 $col_news=M("col_news");
		  switch($sbutton)
		  {
		    ///添加信息内容
		      ...
		      echo "<script>alert('添加成功');window.location.href='/Admin/Caselist/caseimgNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."'</script>";
			  break;
			case "修改":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if($Mlist!==false) 
					{ 
						  ...
						  echo "<script>alert('数据修改成功!');window.location.href='/Admin/Caselist/caseimgNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."'</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
...
function caseColCreate()
{
	   ...
	   $Theobj=M("col_news");
	   $m=M("col_news");
	   $colnews1=$Theobj->where('Col_MenuID=229 and col_State=1')->order('Col_Order asc,Col_ID desc')->select();
	   $colnews2=$Theobj->where('Col_MenuID=230 and col_State=1')->order('Col_Order asc,Col_ID desc')->select();
	   $colnews3=$Theobj->where('Col_MenuID=250 and col_State=1')->order('Col_Order asc,Col_ID desc')->select();
	   $model=M("main_menu");
	   $faID=$this->_request("faID");
	   $ffaID=$this->_request("ffaID");
	   $main_father=getmain_id($faID);
	   ...
	   $this->assign('colnews2',$colnews2);
	   $this->assign('colnews3',$colnews3);
	   switch($act)
	   {
	   	  case "Add": 
	   	      $Col_Order=1;
		 ...
		 $col_news=M("col_news");
		  switch($sbutton)
		  {
		    ///添加信息内容
			  ...
			  //写入日志
			   add_login($ColTitle, $ustate,$content1,$type="添加");
		      echo "<script>alert('数据添加成功!');window.location.href='/Admin/Caselist/caseNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."'</script>";
			  break;
			case "修改":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if($Mlist!==false) 
					{ 
					      //写入日志
						  add_login($ColTitle, $ustate,$content1,$type="修改");
						  echo "<script>alert('数据修改成功!');window.location.href='/Admin/Caselist/caseNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."';</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
...
function linkColCreate()
{
	   ...
	   switch($act)
	   {
		 ...
		 $this->display("linkColCreate");
}
function linkNewsList()
{
	   $language=$this->_request("Language");
	   $ffaID=$_REQUEST["ffaID"];
	   $faID=$this->_request("faID");
	   ...
	   $this->assign('language',$language);
	   $this->assign('faID', $faID);
	   $this->display("linkNewsList");
}


function linkcolcreat_add()
{
		$uploads = $this->_upload();
		 $ttedu_pic = $uploads['image']; 
		 $ttedu_fujian = $uploads['fujian']; 
		 ...
		 $ColEditor=$this->_request("ColEditor");
		 $ColHot=$this->_request("ColHot");
		 $Col_ftitle=$this->_request("Col_ftitle");
		 $ColState=$this->_request("ColState");
		 $ColClick=$this->_request("ColClick");
		 $ColTime=$this->_request("ColTime");
		 $Col_Tel=$this->_request("Col_Tel");
		 ...
		 $Col_gnjj=$this->_request("Col_gnjj");
		 $Col_jszb=$this->_request("Col_jszb");
		 $Col_ft=$this->_request("Col_ft");
		 $ColSource=$this->_request("ColSource");
		 $ColSource=$this->_request("ColSource");
		 $Colxh=$this->_request("Col_xh");
		 $Col_Tel=$this->_request("Col_Tel");
		 $col_news=M("col_news");
		  switch($sbutton)
		  {
		    ///添加信息内容
			  ...
			  //写入日志
			   add_login($ColTitle, $ustate,$content1,$type="添加");
		      echo "<script>alert('数据添加成功!');window.location.href='/Admin/Caselist/linkNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."'</script>";
			  break;
			case "修改":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if($Mlist!==false) 
					{ 
					      //写入日志
						  add_login($ColTitle, $ustate,$content1,$type="修改");
						  echo "<script>alert('数据修改成功!');window.location.href='/Admin/Caselist/linkNewsList?ffaID=".$ffaID."&faID=".$faID."&Language=".$language."';</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
