...
class LinkpicAction extends Action 
{
		 ...
		 $link=M("link");
		  switch($sbutton)
		  {
		    ///添加信息内容
		      ...
		      echo "<script>alert('数据添加成功');window.location.href='/Admin/Linkpic/links_list?faID=".$faID."'</script>";
			  break;
			case "Edit":
			  $id=$this->_request("id");
			  if(!empty($id))
			  {
				  ...
				  if($Mlist!==false) 
					{ 
						...
						echo "<script>alert('数据修改成功!');window.location.href='/Admin/Linkpic/links_list?faID=".$faID."'</script>";
					}
					else
					{ 
						$this->error('数据更新失败！'); 
