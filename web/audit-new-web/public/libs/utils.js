	...
	httpTip.prototype = {
		...
		init:function(obj){
			...
			if(tip.length === 0){
				...
				*/
				//html.push('</div></div>');
				html.push('<div id="_httptip" style="width:100%;height:100%;left:0;right:0;top:0;bottom:0;position:fixed;z-index:999999;display:none;">');
				html.push('<div style="position:relative;width:100%;height:100%;">');
				html.push('<div style="background:url(../public/img/loader.gif) no-repeat center center;background-size:contain;margin-top:-10px;margin-left:-10%;left:50%;top:50%;position:absolute;display:inline-block;width:20px;height:20px;"></div>');
				html.push('</div></div>');
				var $tip = $("#" + this.id);
				if($tip.length == 1){
					$tip.html(html.join(''));
