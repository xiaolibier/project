...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public static final String MODIFY_PASSWORD_URL = "/toModifyPassword";
    public static final Set ignorUrl;
    public static boolean debugMode = false;
    static {
        ignorUrl = new HashSet();
        ignorUrl.add("/login");
        ignorUrl.add("/userLogin");
        ignorUrl.add("/logout");
        String debug = System.getenv().get("AUDIT_DEBUG_MODE");
        if (debug != null && debug.equals("true")) {
            debugMode = true;
            System.out.println("System run in debug mode");
        }
    }
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if (debugMode) {//此部分是测试模式下的使用的代码，和正式系统无关
            ...
            if (userId == null) {
                ...
                Map privilegeIdMap = userService.getUserPrivilegeIdMap(userId);
                user.setUserPrivileges(privilegeIdMap);
                request.getSession().setAttribute(ControllerBase.USER_NAME, user.getName());
                request.getSession().setAttribute(ControllerBase.USER_PRIVILEGES, user.getUserPrivileges());
            }
            return true;
        }
        ...
        if(userId != null) {
            if (servletPath.equals(MODIFY_PASSWORD_URL) || servletPath.equals("/modifyPassword"))
                return true;
            if (UserService.passwordIsDefault(userId)) {
