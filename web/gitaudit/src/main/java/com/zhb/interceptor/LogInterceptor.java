...
public class LogInterceptor implements HandlerInterceptor {
    ...
    public void afterCompletion(HttpServletRequest request,
        ...
        if (uri.indexOf(".") >= 0)
            return;
        logger.debug("[THREAD_END][" + (request).getRequestURI() + "]");
    }

    public void initThreadName(HttpServletRequest request) {
        ...
        logger.debug("[THREAD_BEGIN][" + ip + "][" + (request).getRequestURI() + "][" + userId + "]");
    }
