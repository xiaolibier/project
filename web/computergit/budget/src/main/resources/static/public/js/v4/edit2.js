$(function(){
	...
	g.ss = ['1/3','4/6','7/12','13/19','20/27','28/35','36/39','40/44','45/46'];
	g.nowCity = '';
	g.role = 'viewer';//admin 管理员//user 商务部普通用户//viewer 未登录的用户//reviewer 审核员
	g.state = '';
	g.saveName = 'bepkv4';//识别版本或类型
...
/* ---------------------------set----------------------------------------- */
	...
	function setCity(data){
		var data = data || '';
		var cenNum = 0,zonpinci = 0,zhongx1 = 0,zhongx2 = 0,zhongx3 = 0,zonpinci1 = 0,zonpinci2 = 0,zonpinci3 = 0;
		...
		html +='</table>';
		$('#cityList').html(html);
		cenNum = zhongx1 + zhongx2 + zhongx3;
	...
	function getProTTMoney(){
		...
		var shui = ((parseFloat(jis)+parseFloat(cha))*0.07).toFixed(0);
		$('#shuifei').val(shui);
