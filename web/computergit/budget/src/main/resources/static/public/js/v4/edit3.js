$(function(){
	...
	g.ss = ['1/2','3/7','8/21','22/23','24/25'];
	g.nowCity = '';
	g.role = 'viewer';//admin 管理员//user 商务部普通用户//viewer 未登录的用户//reviewer 审核员
	g.state = '';
	g.saveName = 'gmpv4';//识别版本或类型
	g.printUrl1 = 'edit3.html';
	g.printUrl2 = 'print3.html';
...
/* ---------------------------set----------------------------------------- */
	...
	function setCity(data){
		var data = data || '';
		var cenNum = 0,zonpinci = 0,cropinci = 0;
	...
	function addCity(){
		...
		var clo = _city + '/' + _cen + '/' + jicipin + '/' + way;
		str1.push(clo);
	...
	//计算差旅费用表出差天数
	function getDays(){
		var a0 = parseInt($('.zonyutt3').val() || '0');//3.稽查现场实施
	...
	function setVal(){
		...
		var _val3 = $('.danbinglifangshicishu').val() || '';//获取单病例访视次数数值
		var _val4 = $('.crojichacishu').val() || '';//cro稽查次数之和
		var va = _value.replace(/[^0-9]/g,'');//获取数字部分
		//$('.dy'+_num).val(va);
