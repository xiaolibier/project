$(function(){
	...
	g.ss = ['1/5','6/8','9/13','14/20','21/28','29/32','33/37','38/39'];
	g.nowCity = '';
	g.role = 'viewer';//admin 管理员//user 商务部普通用户//viewer 未登录的用户//reviewer 审核员
	g.state = '';
	g.saveName = 'bepk';//识别版本或类型
...
/* ---------------------------set----------------------------------------- */
	...
	function setCity(data){
		var data = data || '';
		var cenNum = 0,zonpinci = 0,zhongx1 = 0,zhongx2 = 0,zonpinci1 = 0,zonpinci2 = 0;
	...
	function addCity(){
		...
		setCloTable(_city,price,jicipin,by);//生成表三数据
		runAll();//按顺序计算所有表
	}
	//删除城市
	...
	function setTTtime(num){
		...
		//如果相乘的三项都为空 结果清零
		if($('.dy'+num).val() == '' && ($('.cayu'+num).val() || '1') == '1' && $('.py'+num).val() == ''){
			tou = '';
		}
		$('.zonyu'+num).val(tou);
		//实际
