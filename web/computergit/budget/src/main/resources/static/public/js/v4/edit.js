$(function(){
	...
	g.ss = ['1/3','4/6','7/12','13/21','22/30','31/34','35/39','40/41'];
	g.nowCity = '';
	g.role = 'viewer';//admin 管理员//user 商务部普通用户//viewer 未登录的用户//reviewer 审核员
	g.state = '';
	g.saveName = 'abcv4';//识别版本或类型
	g.printUrl1 = 'edit.html';
	g.printUrl2 = 'print.html';
...
/* ---------------------------set----------------------------------------- */
	...
	//计算病例数 从上面弹窗的 2 5 8三项相加获取
	$('.binglione').blur(function(){
		//getBingLiShu();
	});
	
	
	//计算病例数 从上面弹窗的 2 5 8三项相加获取
	function getBingLiShu(){
		var ttBingLiShu = 0;
		$('.binglione').each(function(){//遍历每一个相加项
			var val = parseInt($(this).val() || '0');
			ttBingLiShu += val ;
		})
		if(ttBingLiShu == 0){Utils.alert('病例数不能为空！');return false;}
		$('.binglishu').val(ttBingLiShu);
	}
	...
	function printFunc3(){
		...
		var binglishu = $('.binglishu').val() || '';
		if(binglishu == ''){Utils.alert('病例数不能为空');return false;}
	...
	//判断是不是选中的 全过程控制
	function corQuan(){
		//判断选择的是全过程控制 才显示 费用明细表 1项目管理 模块
		var _c = $(".choisejichaleix").val() || '';
		if(_c == ''){return false;}
		if(_c.indexOf('全过程') > -1){
			$('.tablecell1').show();
		}else{
		...
		var hh = 1;
		$('.nbs').each(function(g){
			if($(this).parents('.tablecell1').css('display') != 'none'){
				var gg = hh++;
				$(this).html(gg);
			}
		})
	...
	function setTable2Val(){
		...
		$('.ffdanjiacommon').val(danjia);//给单价赋值
	...
	function isPrint(){
		...
		//打印总表的时候 筛选总表为零的项 隐藏
		$('.zongbiacommon').each(function(){
			var _ttis = $(this);
			var j = parseInt(_ttis.find('.ffetimecommon').val() || '0');
			var i = parseInt(_ttis.find('.ffttmoneycommon').val() || '0');
			if(j == 0 && i == 0){
				_ttis.hide();
			}
		});
	...
	function setCity(data){
		var data = data || '';
		var cenNum = 0,zonpinci = 0,cropinci = 0,ttzong = 0;
		...
		for(var i=0,len=str1.length;i<len;i++){
			...
			cenNum += c;
			//总稽查次数 等于所有 SITE 项的稽查次数之和
			if(bepk.indexOf('SITE') > -1){zonpinci += d;}
			if(bepk.indexOf('供应商') > -1){cropinci += d;}
			ttzong += d;
			html +='<tr>'
				+'<td>'+_city+'</td>'
				+'<td>'+way+'</td>'
		...
		$('.jichacishu').val(ttzong);
		$('.jichacishun').val(zonpinci);
		$('.crojichacishu').val(cropinci);
	}
	//选择城市时自动带出默认出行方式
	...
	function addCity(){
		...
		setCloTable(_city,price,jicipin,way);
		runAll();//按顺序计算所有表
		$('#cityChoice,#cenNum,#jicipin,#byWay').val('');//清空数据
	}
	//删除城市
	...
	function saveProject(){
		...
		var condi = {};
		condi.type = g.saveName;//存储项目类型 一般表
	...
	function getDays(){
		...
		var tou = ((a0+a+b)/8/d) || 0;
		var tou2 = ((a0+a+b)/8/2/d) || 0;
		//不到一天算一天
		tou = Math.ceil(tou);
		tou2 = Math.ceil(tou2);
		$('.ccdays').val(tou2);//
	...
	function setTTtime(num){
		...
		if(kk.indexOf('分钟/访视/例') > -1 || kk.indexOf('分钟/例') > -1 ){
			fenzhong = true;
		}
		...
		//如果相乘的三项都为空 结果清零
		if($('.dy'+num).val() == '' && ($('.cayu'+num).val() || '1') == '1' && $('.py'+num).val() == '' || $('.py'+num).val() == '' || $('.cayu'+num).val() == ''){
			tou = '';
		}
		$('.zonyu'+num).val(tou);
		//实际
	...
	function everyCaw(){
		...
		resetVal('h/项目','1');
		resetVal('h/供应商',$('.crojichacishu').val());
		resetVal('h/SITE',$('.jichacishun').val());
	}
	//判断更新哪个
	function resetVal(str,val){
		...
		$('.dowchoice').each(function(){//遍历所有表二 行
			...
			if(_value.indexOf('h/月') > -1 && str == 'h/月'){
				...
				$('.py'+_num).val('1');
				//setTTtime(_num);
			}else if(_value.indexOf('h/供应商') > -1 && str == 'h/供应商'){
				$('.py'+_num).val(val);
				//setTTtime(_num);	
			}else if(_value.indexOf('h/SITE') > -1 && str == 'h/SITE'){
				$('.py'+_num).val(val);
				//setTTtime(_num);	
	...
	function getAllsum(num){
		...
		if(_num == ''){
			...
			$('.dowchoice').each(function(){
				var _num = $(this).attr('ste') || '';
				setTTtime(_num);
				getTT(_num,'zonyu',2);//计算费用明细表小结-总时间预期
				getTT(_num,'zonsh',2);//计算费用明细表小结-总时间实际
				getTT(_num,'feyu');//计算费用明细表小结-总费用预期
				getTT(_num,'fesh');//计算费用明细表小结-总费用实际
				//console.log(_num);
			});
		}else{
			setTTtime(_num);
	...
	//费用明细表 每个小结 的 总时间实际-总时间预期-总费用实际-总费用预期
	function getTT(_num,cla,xiaoshu){//a 开始 b 结束 cla class前缀
		...
		//判断稽查类型 如果稽查类型 不是全过程控制 则小结所有总值为零
		if(clas == 1 && $('.choisejichaleix').val().indexOf('全过程') <= -1){_tt = 0;}
		
		//判断有没有稽查 CRO部分 如果没有 小结所有总值归零 
		if(clas == 4 && g.nowCity.indexOf('供应商') <= -1){_tt = 0;}
		//判断有没有稽查 SITE部分 如果没有 小结所有总值归零 
		if(clas == 5&& g.nowCity.indexOf('SITE') <= -1){_tt = 0;}
	...
	$('#xdoc_body').on('blur',".dycommon",function(){
			...
			if(_str.indexOf('h/月') > -1){
				...
				res = val + 'h/次';
			}else if(_str.indexOf('h/供应商') > -1){
				res = val + 'h/供应商';	
			}else if(_str.indexOf('h/SITE') > -1){
				res = val + 'h/SITE';	
	...
	function setVal(){
		...
		//var _val3 = $('.danbinglifangshicishu').val() || '';//获取单病例访视次数数值
		var _val4 = $('.crojichacishu').val() || '';//供应商稽查次数之和
		var _val4 = $('.jichacishun').val() || '';//SITE稽查次数之和
		var va = _value.replace(/[^0-9]/g,'');//获取数字部分
		//$('.dy'+_num).val(va);
		if(_value.indexOf('h/月') > -1){
			...
			$('.py'+_num).val('1');
		}else if(_value.indexOf('h/供应商') > -1){
			$('.py'+_num).val(_val4);
		}else if(_value.indexOf('h/SITE') > -1){
			$('.py'+_num).val(_val4);
